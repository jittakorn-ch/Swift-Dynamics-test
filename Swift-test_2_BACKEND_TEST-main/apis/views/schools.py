...
class StudentSubjectsScoreAPIView(APIView):
    ...
    def post(request, *args, **kwargs):
        ...
        # Payload data must be contained `first_name`, `last_name`, `subject_title` and `score`
        if None in [student_first_name, student_last_name, subject_title, score]:
            return Response("Payload data is incomplete.", status=status.HTTP_400_BAD_REQUEST)

            ## `first_name` in payload must be string (if not return bad request status).
        if not isinstance(student_first_name, str):
            return Response("Student's first name must be a string.", status=status.HTTP_400_BAD_REQUEST)
        
            ## `last_name` in payload must be string (if not return bad request status).
        if not isinstance(student_last_name, str):
            return Response("Student's last name must be a string.", status=status.HTTP_400_BAD_REQUEST)

            ## `subject_title` in payload must be string (if not return bad request status).
        if not isinstance(subject_title, str):
            return Response("Subject's title must be a string.", status=status.HTTP_400_BAD_REQUEST)
        
            ## `subject_title` in payload must be string (if not return bad request status).
        # Score must be number, equal or greater than 0 and equal or less than 100.
        if not isinstance(score, (int, float)) or not (0 <= score <= 100):
            return Response("Score must be a number and must be between 0 and 100.", status=status.HTTP_400_BAD_REQUEST)
        

        personnel = Personnel.objects.all()
        subjects = Subjects.objects.all()

        matching_name = Personnel.objects.filter(
            first_name=student_first_name,
            last_name=student_last_name
        )

        matching_subject = Subjects.objects.filter(title=subject_title)

        if not matching_name.exists():
            return Response("Not found student name.", status=status.HTTP_400_BAD_REQUEST)
        
        if not matching_subject.exists():
            return Response("Not found subject.", status=status.HTTP_400_BAD_REQUEST)


        student = get_object_or_404(Personnel, first_name=student_first_name, last_name=student_last_name)
        subject = get_object_or_404(Subjects, title=subject_title)
        ...
        try:
            ...
            return JsonResponse({"message": "Score updated successfully.",
                                 "student_details": {
                                     ...
                                     "personnel_type": student.get_personnel_type_display()
                                 },
                                 "subject_title": subject.title,
                                 "credit": score_obj.credit,
                                 "score": score_obj.score}, status=201)
            ...
            for mapping in credits_mapping:
                if mapping["subject_id"] == subject.id:
                    ...
                    credit = next((c["credit"] for c in credits_context if c["id"] == credit_id), None)
                    break

            if credit is None:
                return JsonResponse({"message": "Subject's credit not found in the database."}, status=400)

            score_obj = StudentSubjectsScore.objects.create(student=student, subjects=subject, credit=credit, score=score)
...
class StudentSubjectsScoreDetailsAPIView(APIView):
    ...
    def get(request, *args, **kwargs):
        ...
        student_data = {
         ...
         # Retrieve subject details, scores, and grades
        subject_scores = StudentSubjectsScore.objects.filter(student=student)

        # check registration
        if not subject_scores.exists():
            return Response("No registration information.", status=status.HTTP_400_BAD_REQUEST)

        subject_data = []
        score_x_credit = 0
        total_credit = 0

        for index in subject_scores:
            subject = index.subjects
            score = index.score
            credit = index.credit
            grade, value = get_grade(index.score)
            subject_data.append({
            ...
            score_x_credit += credit*value
            total_credit += credit

        # Calculate grade point average
        grade_point_average = score_x_credit / total_credit
        grade_point_average = round(grade_point_average, 2)     # set decimal
...
class PersonnelDetailsAPIView(APIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        personnel = Personnel.objects.select_related('school_class__school').order_by(
            'personnel_type',
        ...
        if school_title:
            personnel = personnel.filter(school_class__school__title__iexact=school_title)

        response_data = []

        for order, person in enumerate(personnel, start=1):
            school = person.school_class.school.title
            role = person.get_personnel_type_display()
            class_order = person.school_class.class_order
            name = f"{person.first_name.capitalize()} {person.last_name.capitalize()}"

            data = f"{order}. school: {school}, role: {role}, class: {class_order}, name: {name}"
...
class SchoolHierarchyAPIView(APIView):
    ...
    def get(request, *args, **kwargs):
        ...
        for school in schools:
            school_classes = Classes.objects.filter(school=school)
            class_data = {}

            for school_class in school_classes:
                personnel = Personnel.objects.filter(school_class=school_class)
                teachers = personnel.filter(personnel_type=0)
                head_of_the_room = personnel.filter(personnel_type=1)
                students = personnel.filter(personnel_type=2)

                teacher_data = []
                for teacher in teachers:
                    teacher_data.append({"Teacher: {} {}".format(teacher.first_name, teacher.last_name)})

                head_of_the_room_data = []
                for head in head_of_the_room:
                    head_of_the_room_data.append({"Head of the room": "{} {}".format(head.first_name, head.last_name)})

                student_data = []
                for student in students:
                    student_data.append({"Student": "{} {}".format(student.first_name, student.last_name)})

                class_data["class {}".format(school_class.class_order)] = teacher_data + head_of_the_room_data + student_data
...
class SchoolStructureAPIView(APIView):
    ...
    def get(request, *args, **kwargs):
        ...
        # Retrieve all SchoolStructure objects from the database
        school_structure = SchoolStructure.objects.all()

        room_names = defaultdict(list)

        parent_ids = [2, 10, 18, 27, 35, 43]
        response_data = []

        for i in school_structure:
            if i.parent_id in parent_ids:
                room_names[i.parent_id].append({"title": i.title})
                
        level = 1        
        for parent_id in parent_ids:
            response_data.append({
                "title": "มัธยมต้น" if parent_id <= 18 else "มัธยมปลาย",
                "sub": [
                    {
                        "title": "ม." + str(level),
                        "sub": room_names[parent_id]
                    }
                ]
            })
            level += 1
